...
import app.pay.paypanda.databinding.DialogFetchBillBinding
import app.pay.paypanda.databinding.FragmentFetchBillPaymentBinding
...
import app.pay.paypanda.responsemodels.billpayresponse.BillPayResponse
import app.pay.paypanda.responsemodels.fetchBill.FetchBillResponse
import app.pay.paypanda.responsemodels.getOperators.CustomerParam
...
class FetchBillPayment : BaseFragment<FragmentFetchBillPaymentBinding>(FragmentFetchBillPaymentBinding::inflate), OnBackPressedListner {
    ...
    private var catName = ""
    private var catID = ""
    private lateinit var inputData: MutableList<app.pay.paypanda.helperclasses.CustomerParam>
    override fun init() {
        nullActivityCheck()
    ...
    private fun getInputParams(): List<app.pay.paypanda.helperclasses.CustomerParam> {
        inputData = mutableListOf()
        inputData.add(app.pay.paypanda.helperclasses.CustomerParam(params?.get(0)?.paramName.toString(), binding.edtAd1.text.toString()))

        if (params?.size!! > 1 && params?.get(1)?.optional == false && binding.edtAd2.text.toString().isNotEmpty()) {
            inputData.add(app.pay.paypanda.helperclasses.CustomerParam(params?.get(1)?.paramName.toString(), binding.edtAd2.text.toString()))
        }

        if (params?.size!! > 2 && params?.get(2)?.optional == false && binding.edtAd3.text.toString().isNotEmpty()) {
            inputData.add(app.pay.paypanda.helperclasses.CustomerParam(params?.get(2)?.paramName.toString(), binding.edtAd3.text.toString()))
        }

        if (params?.size!! > 3 && params?.get(3)?.optional == false && binding.edtAd4.text.toString().isNotEmpty()) {
            inputData.add(app.pay.paypanda.helperclasses.CustomerParam(params?.get(3)?.paramName.toString(), binding.edtAd4.text.toString()))
        }

        if (params?.size!! > 4 && params?.get(4)?.optional == false && binding.edtAd5.text.toString().isEmpty()) {
            inputData.add(app.pay.paypanda.helperclasses.CustomerParam(params?.get(4)?.paramName.toString(), binding.edtAd5.text.toString()))
        }
