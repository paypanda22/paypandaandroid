...
import app.pay.paypanda.activity.LoginRegistrationActivity
import app.pay.paypanda.responsemodels.login.Data
import app.pay.paypanda.retrofit.Constant
import app.pay.paypanda.retrofit.Constant.PREFER_NAME
...
class UserSession(private val context: Context) {
    ...
    fun setUserData(data: app.pay.paypanda.responsemodels.dashBoardData.Data):Boolean{
        val wasSaved = editor.apply {
            putBoolean(Constant.IS_USER_LOGIN, true)
            putString(Constant.NAME,data.name)
            putString(Constant.PAN_NAME,data.pan_name)
