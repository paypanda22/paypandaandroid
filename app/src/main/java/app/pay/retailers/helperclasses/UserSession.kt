...
import app.pay.retailers.activity.LoginRegistrationActivity
import app.pay.retailers.responsemodels.login.Data
import app.pay.retailers.retrofit.Constant
import app.pay.retailers.retrofit.Constant.PREFER_NAME
...
class UserSession(private val context: Context) {
    ...
    fun setUserData(data: app.pay.retailers.responsemodels.dashBoardData.Data):Boolean{
        val wasSaved = editor.apply {
            putBoolean(Constant.IS_USER_LOGIN, true)
            putString(Constant.NAME,data.name)
            putString(Constant.PAN_NAME,data.pan_name)
         ...
         //   data.notification?.let { putInt(Constant.NOTIFICATION_COUNT, it) }
           ...
           // putString(Constant.USERTYPE,data.user_type_id?.user_type.toString())
            ...
            putString(Constant.MERCHANT_CODE,data.refer_id.toString())
